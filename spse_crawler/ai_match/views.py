...
def api_match_results(request):
    ...
    for m in qs[:100]:
        items.append({
            ...
            "eligibility_status": getattr(m, "eligibility_status", "ELIGIBLE"),
            "mandatory_passed": getattr(m, "mandatory_passed", True),
            "blockers": getattr(m, "blockers", []),
            "missing_requirements": getattr(m, "missing_requirements", []),
            "recommended_actions": getattr(m, "recommended_actions", []),
            "matcher_version": getattr(m, "matcher_version", "v0.2"),
            "summary": m.summary,
